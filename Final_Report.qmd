...
**2.1.1.2 Creating tibbles for different cases**

***Initial Survey/March, 2010***

This data sets contains both partisipants' demographics and responses to survey items. Cases for this data set are (participant) and (participant, month).
...
# combine the columns into group type, interaction, and group_number

apr_group <- apr_group |>
  pivot_longer(
    cols = -participantID,
    names_to = c("group_type", "interaction", "group_number"),
    names_pattern = "(Formal|Informal)groups(\\w+)\\.Group(\\d+)",
...
```{r}
# replace multiple dots with single dot in the column names
...
mar_group <- mar_group |>
 pivot_longer(
    cols = -participantID,
    names_to = c("group_type", "interaction", "group_number"),
    names_pattern = "(Formal|Informal)\\.group[s]?\\.([a-z\\.]+)\\.Group\\.(\\d+)",
    values_to = "group_name"
  ) |>
  filter(!is.na(group_name) & group_name != "") |> 
